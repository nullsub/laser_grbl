...
void limits_init() {
  LIMIT_XY_PORT->FIODIR &= ~(LIMIT_MASK);  // set as input
  // note: pullup-resistores enabled by default on the LPC1768
  
  LIMIT_OVERWRITE_PORT->FIODIR |= (1<<LIMIT_OVERWRITE_BIT);  // set as output
...
void limit_overwrite_enable() {
  ...
  LIMIT_OVERWRITE_PORT->FIOCLR = (1<<LIMIT_OVERWRITE_BIT);
}

void limit_overwrite_disable() {
  LIMIT_OVERWRITE_PORT->FIOSET = (1<<LIMIT_OVERWRITE_BIT);  
}
